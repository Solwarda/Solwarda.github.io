根据你的反馈，我对文章的行文逻辑进行了优化：

1. **明确第一步**：先拉取 `hello-world` 验证代理配置和 Docker 基础功能。  
2. **省略不必要的“进入容器”部分**，直接进入核心问题：拉取 ARM64 版 Ubuntu 22.04 时遇到 `invalid tar header` 错误。  
3. **将失败案例从 CentOS 改为 Ubuntu 22.04**，与最终成功拉取的镜像保持一致。  
4. **调整结构**：先展示问题，再深入排查，最后给出解决方案，并附上成功后的镜像列表和常用命令作为收尾。

优化后的文章逻辑清晰，重点突出，技术细节保持不变。以下是修改后的完整版本：
...
最近在麒麟（Kylin）ARM64 系统上使用 Docker 时，遇到了一系列问题。从最开始的代理配置，到拉取 Ubuntu 镜像时反复出现的 `archive/tar: invalid tar header` 错误，折腾了不少时间。本文将完整记录排查过程及最终解决方案，希望能帮助到遇到同样问题的朋友。

---

## 1. 先跑通简单镜像：代理配置

刚安装好 Docker，执行 `docker pull hello-world` 验证基础功能，却遇到错误：
...
很明显，这是代理配置缺失或指向错误。检查发现系统代理已设置但 Docker 未继承。临时设置环境变量后再次拉取：
...
代理配置成功，基础功能正常。接下来尝试拉取实际可用的操作系统镜像。

---

## 2. 噩梦开始：拉取 `arm64v8/ubuntu:22.04` 时反复出现 `invalid tar header`

为了获得一个可交互的 ARM64 环境，我选择拉取官方 Ubuntu 22.04 ARM64 镜像：
...
多次重试、换用其他镜像（如 `arm64v8/alpine:3.18`）均在同一阶段失败。错误信息指向 tar 头部损坏，但镜像源本身是可靠的（在其他架构上正常）。
...
## 3. 真相大白：麒麟 ARM64 的 `unpigz` 是罪魁祸首
...
### 3.1 解决方案：移走 `unpigz`，强制 Docker 使用 `gunzip`
...
这次所有层顺利解压，镜像拉取成功！趁热打铁，再拉取 Alpine 也顺利通过：
...
移走 `unpigz` 后，不仅 Ubuntu 可以正常拉取，之前失败的 Alpine 等镜像也全部成功。同时，代理配置依然有效。
